...
class POMA(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            with autocast():
                logits_matrix, _, _ = self.model(image)
    ...
    def split_dataset_by_bisimilarities(self, epoch):
        ...
        for cls in range(self.num_classes):
            ...
            # fit a two-component GMM to the robust similarity
            gmm = GaussianMixture(n_components=2,max_iter=10,tol=1e-2,reg_covar=5e-4)
            gmm.fit(robust_similarity)
            prob = gmm.predict_proba(robust_similarity)
            prob = prob[:, gmm.means_.argmax()]

            # select the trainng samples
            thredhold = 0.5
            for row in range(clsbag_cossim.shape[0]):
                ip = cls_impath[row]
                ip = './data/' + ip.split('/data/')[1]
                if prob[row] >= thredhold:
        ...
        if total == 0:
            ...
            clean_rate = tp / total
        print(f"epoch [{epoch+1}]     total samples: {total}    tp samples: {tp}    clean rate: {clean_rate:.4f}")
